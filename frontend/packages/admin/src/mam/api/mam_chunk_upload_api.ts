...
export function createChunkSession(data: MaterialChunkUploadSessionCreateDTO): Promise<MaterialChunkUploadSessionVO> {
  return api.post('/api/material/upload/chunk/session', data, { timeout: UPLOAD_TIMEOUT_MS })
}

export function getChunkSession(sessionId: string): Promise<MaterialChunkUploadSessionVO> {
  return api.get(`/api/material/upload/chunk/session/${encodeURIComponent(sessionId)}`)
}
...
export function reportChunkPart(
  ...
  return api.post(`/api/material/upload/chunk/session/${encodeURIComponent(sessionId)}/part`, data, { timeout: UPLOAD_TIMEOUT_MS })
}

export function completeChunkSession(sessionId: string): Promise<MaterialChunkUploadSessionVO> {
  return api.post(`/api/material/upload/chunk/session/${encodeURIComponent(sessionId)}/complete`, {}, { timeout: UPLOAD_TIMEOUT_MS })
}

export function cancelChunkSession(sessionId: string): Promise<MaterialChunkUploadSessionVO> {
  return api.post(`/api/material/upload/chunk/session/${encodeURIComponent(sessionId)}/cancel`)
}
...
export function uploadChunkWithHttpHeaders(
  ...
  return api.post(`/api/material/upload/chunk/session/${encodeURIComponent(sessionId)}/chunk`, payload, {
    headers,
