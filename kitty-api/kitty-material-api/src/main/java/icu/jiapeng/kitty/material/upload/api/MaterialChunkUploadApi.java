...
public interface MaterialChunkUploadApi {
    ...
    MaterialChunkUploadSessionVO getSession(@PathVariable("sessionId") String sessionId);
    ...
    MaterialChunkUploadSessionVO completeSession(@PathVariable("sessionId") String sessionId);

    @Operation(summary = "取消会话并清理已登记分片")
    @PostMapping("/api/material/upload/chunk/session/{sessionId}/cancel")
    MaterialChunkUploadSessionVO cancelSession(@PathVariable("sessionId") String sessionId);
