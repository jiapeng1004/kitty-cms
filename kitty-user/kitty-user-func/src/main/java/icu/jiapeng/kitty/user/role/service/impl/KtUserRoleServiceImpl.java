...
import icu.jiapeng.kitty.user.role.entity.KtUserRoleUser;
...
@Service
public class KtUserRoleServiceImpl extends ServiceImpl<KtUserRoleMapper, KtUserRole> implements KtUserRoleService {
    @Resource
    ...
    public List<String> getRoleIdsByUserId(String string) {
        // 获取用户的角色
        return ktUserRoleUserService.lambdaQuery().select(KtUserRoleUser::getRoleId).eq(KtUserRoleUser::getUserId, string)
                .list().stream().map(KtUserRoleUser::getRoleId).toList();
    }

    @Override
    public RoleDetailInfoVo detail(String roleId) {
        KtUserRole role = getById(roleId);
