...
// DeleteByID 物理删除任务记录，与 Java deleteById 对齐
func (r *TaskRepo) DeleteByID(id string) (bool, error) {
	result := r.db.Where("id = ?", id).Delete(&model.TranscodeTask{})
	return result.RowsAffected > 0, result.Error
}

// Save 保存或更新任务记录
func (r *TaskRepo) Save(t *model.TranscodeTask) error {
