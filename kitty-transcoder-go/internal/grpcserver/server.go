...
func (s *TranscodeGrpcServer) GetStrategies(ctx context.Context, _ *grpcpb.GetStrategiesReq) (*grpcpb.GetStrategiesResp, error) {
	list, err := s.strategySvc.ListStrategiesVO()
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	strategies := make([]*grpcpb.StrategySummary, 0, len(list))
	for i := range list {
		strategies = append(strategies, &grpcpb.StrategySummary{Id: list[i].ID, Name: list[i].Name})
	}
	return &grpcpb.GetStrategiesResp{Strategies: strategies}, nil
}
...
func (s *TranscodeGrpcServer) GetStrategy(ctx context.Context, req *grpcpb.GetStrategyReq) (*grpcpb.StrategyResp, error) {
	...
	return &grpcpb.StrategyResp{Id: req.GetStrategyId(), Name: name, Steps: dtos}, nil
}
...
func (s *TranscodeGrpcServer) CreateStrategy(ctx context.Context, req *grpcpb.CreateStrategyReq) (*grpcpb.CreateStrategyResp, error) {
	...
	rootID, err := s.strategySvc.CreateStrategy(req.GetName(), steps)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	return &grpcpb.CreateStrategyResp{Id: rootID}, nil
}
