...
func (h *TaskHandler) Register(group *ghttp.RouterGroup) {
	...
	group.DELETE("/task/:id/record", h.deleteRecordWrap)
...
// deleteRecordWrap DELETE /task/:id/record 物理删除任务记录，与 Java deleteTask 对齐
func (h *TaskHandler) deleteRecordWrap(r *ghttp.Request) {
	id := r.Get("id").String()
	if id == "" {
		r.Response.WriteStatus(400)
		r.Response.Header().Set("Content-Type", "application/json; charset=UTF-8")
		r.Response.WriteJson(g.Map{"error": "task id required"})
		return
	}
	ok, err := h.svc.DeleteTask(id)
	if err != nil {
		r.Response.WriteStatus(500)
		r.Response.Header().Set("Content-Type", "application/json; charset=UTF-8")
		r.Response.WriteJson(g.Map{"error": err.Error()})
