...
func (h *StrategyHandler) Register(group *ghttp.RouterGroup) {
	...
	group.GET("/strategy/:id/export", h.export)
}
...
func (h *StrategyHandler) create(r *ghttp.Request) {
	...
	// 返回策略 ID（root_id，long）
	r.Response.WriteJson(rootID)
}
...
func (h *StrategyHandler) get(r *ghttp.Request) {
	...
	if id == 0 {
		r.Response.WriteStatus(400)
		r.Response.Header().Set("Content-Type", "application/json; charset=UTF-8")
		r.Response.WriteJson(g.Map{"error": "invalid strategy id"})
		return
	}
...
func (h *StrategyHandler) export(r *ghttp.Request) {
	id := r.Get("id").Int64()
	if id == 0 {
		r.Response.WriteStatus(400)
		r.Response.Header().Set("Content-Type", "application/json; charset=UTF-8")
	...
	yamlContent, err := h.svc.ExportStrategy(id)
	if err != nil {
		r.Response.WriteStatus(500)
		r.Response.Header().Set("Content-Type", "application/json; charset=UTF-8")
		r.Response.WriteJson(g.Map{"error": err.Error()})
	...
	r.Response.Header().Set("Content-Disposition", "attachment; filename=strategy-"+fmt.Sprintf("%d", id)+".yaml")
	r.Response.Write(yamlContent)
}
